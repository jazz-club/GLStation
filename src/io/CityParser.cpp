...
std::string CityParser::osmWayName(const OSMWay &way,
  ...
  if (way.tags.count("operator") && !way.tags.at("operator").empty())
    return sanitizeName(way.tags.at("operator") + "_" + std::to_string(way.id));
...
bool CityParser::importCity(const std::string &cityName) {
  ...
  for (size_t i = 0; i < elements.size(); ++i) {
    ...
    if (type == "node") {
      ...
      if (node.tags.count("power") && node.tags["power"] == "substation") {
  ...
  if (substations.empty()) {
    std::cerr << "No substations found for that area. The name may not match "
...
double CityParser::calculateDistance(double lat1, double lon1, double lat2,
  ...
  double a = sin(dLat / 2) * sin(dLat / 2) + cos(lat1 * Core::PI / 180.0) *
                                                 cos(lat2 * Core::PI / 180.0) *
