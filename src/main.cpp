...
static void printLiveDashboard(const GLStation::Simulation::Engine &engine,
  ...
  for (const auto &sub : engine.getSubstations()) {
    for (const auto &comp : sub->getComponents()) {
      if (auto *node = dynamic_cast<GLStation::Grid::Node *>(comp.get())) {
      ...
      if (auto *line = dynamic_cast<GLStation::Grid::Line *>(comp.get())) {
        double limit = line->getCurrentLimit();
        if (limit > 1e-6) {
          double pct = 100.0 * std::abs(line->getCurrentFlow()) / limit;
          if (pct > worstLinePct) {
            worstLinePct = pct;
      ...
      if (auto *trafo =
        ...
        if (limit > 1e-6) {
          double pct = 100.0 * std::abs(trafo->getCurrentFlow()) / limit;
          if (pct > worstLinePct) {
            worstLinePct = pct;
  ...
  l1 << " T " << engine.getTickCount() << " ms  "
     << (s_ansiEnabled ? freqColour : "") << std::fixed << std::setprecision(2)
     << freq << " Hz" << (s_ansiEnabled ? ANSI_RESET : "");
  ...
  std::cout << "  |" << pad(" f " + sparkline + (sparkline.empty() ? "-" : ""))
            << "|\n";
...
int main() {
  try {
    ...
    std::string line;
    while (true) {
      std::cout << "[" << engine.getTickCount() << " ms] > " << std::flush;
      ...
      if (cmd == "exit")
        ...
        std::cout
            ...
            << "export [filename]\n"
            << "scenario list\n"
            << "scenario add <open|close|set_load|set_gen> <tick> <id> [kW]\n"
            << "import <name>\n"
            << std::endl;
      } else if (cmd == "tick") {
        std::string arg;
        ...
        if (infiniteMode) {
          ...
          if (!intStr.empty())
            try {
              intervalTicks =
                  static_cast<GLStation::Core::u64>(std::stoull(intStr));
            } catch (...) {
            }
          if (intervalTicks < 1)
            intervalTicks = 1;
          ...
          for (;;) {
            for (GLStation::Core::u64 i = 0; i < intervalTicks; ++i)
              engine.tick();
            printLiveDashboard(engine, !first, true, engine.getTickCount(), 0);
            first = false;
          }
        } else {
          ...
          for (; done < durationTicks;) {
            GLStation::Core::u64 chunk = (durationTicks - done > intervalTicks)
                                             ? intervalTicks
                                             : (durationTicks - done);
          ...
          auto ms =
              std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0)
                  .count();
          std::cout << "\nDone. " << durationTicks << " ticks in " << ms
                    << "ms." << std::endl;
        }
      } else if (cmd == "status") {
        ...
        bool found = false;
        std::vector<std::pair<GLStation::Core::u64, std::string>> partial;
          ...
          for (const auto &comp : sub->getComponents()) {
            ...
            if (name.find(query) != std::string::npos) {
              std::cout << "  - [ID: " << std::setw(2) << comp->getId() << "] "
                        << std::setw(15) << name << " (" << typeid(*comp).name()
                        << ")" << std::endl;
              found = true;
            } else if (qLower.size() >= 2 &&
                       lower(name).find(qLower) != std::string::npos)
        ...
        for (const auto &sub : engine.getSubstations()) {
          std::cout << "\n[Substation: " << sub->getName() << "]" << std::endl;
          ...
          for (const auto &comp : sub->getComponents()) {
            if (auto gen =
                    dynamic_cast<GLStation::Grid::Generator *>(comp.get()))
              nodeMap[gen->getConnectedNode()].push_back(comp.get());
            else if (auto load =
                         dynamic_cast<GLStation::Grid::Load *>(comp.get()))
            ...
            } else if (auto breaker = dynamic_cast<GLStation::Grid::Breaker *>(
                           comp.get())) {
          ...
          for (auto const &[node, components] : nodeMap) {
            ...
            for (auto *comp : components) {
              std::cout << "  |   +-- [ID: " << std::setw(2) << comp->getId()
                        << "] " << comp->getName();
              if (auto b = dynamic_cast<GLStation::Grid::Breaker *>(comp))
        ...
        if (type.empty()) {
          ...
          std::cout << "\nListing [" << type << "] components:" << std::endl;
            ...
            for (const auto &sub : engine.getSubstations()) {
              for (const auto &comp : sub->getComponents()) {
                ...
                if (type == "trafo" &&
                  ...
                  match = true;
                if (type == "breaker" &&
                    dynamic_cast<GLStation::Grid::Breaker *>(comp.get()))
                  match = true;
                if (match) {
              ...
              if (comp->getId() == id) {
                ...
                if (auto line =
                  ...
                  std::cout << "Details:  P_target=" << gen->getTargetP()
                            ...
                            << std::endl;
                } else if (auto load = dynamic_cast<GLStation::Grid::Load *>(
                               comp.get())) {
        ...
        if (ss >> id >> param >> val) {
          ...
          for (const auto &sub : engine.getSubstations()) {
            for (const auto &comp : sub->getComponents()) {
              if (comp->getId() == id) {
                if (auto gen = dynamic_cast<GLStation::Grid::Generator *>(
                        comp.get())) {
                  if (param == "p") {
                    gen->setTargetP(val);
                    ok = true;
                ...
                } else if (auto line = dynamic_cast<GLStation::Grid::Line *>(
                               comp.get())) {
                  if (param == "limit") {
            ...
            if (!hasId)
              ...
              std::cout
                  ...
                  << "' not allowed for this component."
                     "(gen: p,v; load: p,pf; line/trafo: limit; trafo: tap)"
                  << std::endl;
          }
        } else
        ...
        if (sub == "list") {
          ...
          ss >> type >> tick >> id;
          if (type == "open") {
            engine.getScenarioManager().addEvent(tick, [&engine, id]() {
              if (engine.openBreakerById(id))
                ...
                std::cout << "\n[SCENARIO] Breaker ID " << id << " not found.\n"
                          << std::flush;
            });
            std::cout << "At T+" << tick << " ms will open breaker " << id
                      << "." << std::endl;
          } else if (type == "close") {
            engine.getScenarioManager().addEvent(tick, [&engine, id]() {
            ...
            std::cout << "At T+" << tick << "ms will close breaker " << id
                      << "." << std::endl;
          } else if (type == "set_load") {
            double kw;
            if (ss >> kw) {
              engine.getScenarioManager().addEvent(tick, [&engine, id, kw]() {
                if (engine.setLoadPowerById(id, kw))
                  std::cout << "\n[SCENARIO] Load " << id << " set to " << kw
                            << " kW at T=" << engine.getTickCount() << " ms.\n"
                            << std::flush;
                else
                  std::cout << "\n[SCENARIO] Load ID " << id << " not found.\n"
                            << std::flush;
              });
              std::cout << "At T+" << tick << " ms will set load " << id
                        << " to " << kw << " kW." << std::endl;
            } else
              std::cout << "Error: scenario add set_load <tick> <load_id> <kW>"
                        << std::endl;
            ...
            if (ss >> kw) {
              engine.getScenarioManager().addEvent(tick, [&engine, id, kw]() {
                if (engine.setGenTargetPById(id, kw))
                  std::cout << "\n[SCENARIO] Generator " << id << " setpoint "
                            << kw << " kW at T=" << engine.getTickCount()
                            << " ms.\n"
                            << std::flush;
                else
                  std::cout << "\n[SCENARIO] Generator ID " << id
                            << " not found.\n"
                            << std::flush;
              });
              std::cout << "At T+" << tick << " ms will set generator " << id
                        << " to " << kw << " kW." << std::endl;
            } else
              std::cout << "scenario add set_gen <tick> <gen_id> <kW>"
                        << std::endl;
          } else
            std::cout << "scenario add <open|close|set_load|set_gen> "
                         "<tick> <id> [kW]."
                      << std::endl;
        } else if (!sub.empty())
          std::cout << "Use scenario add ... or scenario list." << std::endl;
        else
          std::cout
              << "Usage: scenario list | scenario add <type> <tick> <id> [kW]"
              << std::endl;
      } else if (cmd == "import") {
        std::string cityName;
        ...
        trim(cityName);
        if (!cityName.empty()) {
          if (GLStation::Simulation::CityParser::importCity(cityName)) {
            engine.initialise();
            std::cout << "Grid loaded for: " << cityName << std::endl;
          } else {
            auto suggestions =
                GLStation::Simulation::CityParser::getSuggestions(cityName);
              ...
              if (std::getline(std::cin, choice)) {
                ...
                if (idx >= 1 && idx <= suggestions.size()) {
                  if (GLStation::Simulation::CityParser::importCity(
                          suggestions[idx - 1])) {
                    engine.initialise();
                    std::cout << "Grid loaded for: " << suggestions[idx - 1]
                              << std::endl;
                  }
                }
              }
            } else {
              std::cout << "No matching area in OpenStreetMap or internet "
                           "error idk didnt account for that."
                        << std::endl;
              std::cout << "Use 'import <name>' with exact spelling."
                        << std::endl;
            }
          }
        } else {
        ...
        std::cout << "Error: Unknown Vommand '" << cmd << "'." << std::endl;
      }
    }
