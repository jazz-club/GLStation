      ...
      while (std::getline(file, line)) {
        ...
        if (type == "NODE") {
          ...
          if (ctx.nodes.count(from) && ctx.nodes.count(to)) {
            auto lineObj = std::make_shared<Grid::Line>(name, ctx.nodes[from],
                                                        ctx.nodes[to], r, x);
          ...
          if (ss >> name >> node >> modeStr >> p >> v >> qmin >> qmax) {
            for (auto &c : modeStr)
              c = std::tolower(c);
            if (ctx.nodes.count(node)) {
          ...
          ss >> name >> from >> to;
          if (ctx.nodes.count(from) && ctx.nodes.count(to)) {
            auto brk = std::make_shared<Grid::Breaker>(name, ctx.nodes[from],
                                                       ctx.nodes[to]);
...
void Engine::createDemoGrid() {
  ...
  auto load2 = std::make_shared<Grid::Load>("CityCentre", bus2.get(), 20000.0);
  auto load3 =
...
void Engine::processProtectionRelays() {
      ...
      if (auto line = dynamic_cast<Grid::Line *>(comp.get())) {
        ...
        if (iMag > line->getCurrentLimit()) {
          for (auto brk : breakers) {
            if (!brk->isOpen() && (brk->getFromNode() == line->getFromNode() ||
                                   brk->getToNode() == line->getToNode())) {
              brk->setOpen(true);
              std::cout << "\n[RELAY] OVERCURRENT on " << line->getName()
                        << " (" << std::fixed << std::setprecision(1) << iMag
                        << " A > " << line->getCurrentLimit()
                        << " A limit). Tripping breaker: " << brk->getName()
                        << "\n";
              break;
            }
          }
  ...
  for (auto &stage : s_uflsStages) {
    if (!stage.triggered && m_systemFrequency <= stage.freqThresholdHz) {
      ...
      std::sort(activLoads.begin(), activLoads.end(), [](auto *a, auto *b) {
        return a->getCurrentConsumption() > b->getCurrentConsumption();
      });

      std::cout << "\n[UFLS] Frequency " << std::fixed << std::setprecision(2)
    ...
    if (stage.triggered && m_systemFrequency > stage.freqThresholdHz + 0.3) {
      ...
      std::cout << "\n[UFLS] Frequency recovered to " << std::fixed
                ...
                << " Hz. Restoring loads at stage " << stage.freqThresholdHz
                << " Hz.\n";
    }
  }
}
