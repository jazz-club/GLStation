...
void PowerSolver::buildYBus(
  ...
  for (const auto &sub : substations) {
    for (const auto &comp : sub->getComponents()) {
      ...
      } else if (auto breaker = dynamic_cast<Grid::Breaker *>(comp.get())) {
  ...
  for (auto line : allLines) {
    ...
    std::complex<Core::f64> y = (std::abs(z_pu) > 1e-9)
                                    ? (std::complex<Core::f64>(1.0, 0.0) / z_pu)
                                    : 0.0;
  ...
  s_yBus = std::make_unique<Util::SparseMatrix<std::complex<Core::f64>>>(n, n);
...
bool PowerSolver::runIteration() {
  ...
  std::vector<std::vector<Core::f64>> J(dim, std::vector<Core::f64>(dim, 0.0));
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    for (size_t k = rowPtr[i]; k < rowPtr[i + 1]; ++k) {
      ...
      if (i == j) {
        ...
        J[row_theta][col_theta] = Vi * Vj * (Gij * sin(dij) - Bij * cos(dij));
        if (v_map.count(i)) {
          size_t row_v = v_map[i];
          J[row_v][col_theta] = -Vi * Vj * (Gij * cos(dij) + Bij * sin(dij));
          if (v_map.count(j)) {
            size_t col_v = v_map[j];
            J[row_theta][col_v] = Vi * (Gij * cos(dij) + Bij * sin(dij));
            J[row_v][col_v] = Vi * (Gij * sin(dij) - Bij * cos(dij));
          }
